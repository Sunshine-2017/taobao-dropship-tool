...
const PROFILE_DIR = 'D:\\software\\AI\\ClaudeCode\\taobao-dropship-tool\\server\\data\\clean-profile';

console.log('Launching Edge with clean profile...');
...
if (page.url().includes('login')) {
  console.log('LOGIN NEEDED. Will wait 10 minutes. Please scan QR in the Edge window.');
  console.log('After login, diagnosis will auto-run.');
  try {
...
const iframes = await page.evaluate(() => {
  return Array.from(document.querySelectorAll('iframe')).map(f => ({
    ...
    id: f.id || '',
    className: (f.className || '').substring(0, 40),
  }));
});
