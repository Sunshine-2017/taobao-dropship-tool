...
function rowToSource(row: Record<string, unknown>): SourceProduct {
  return row as unknown as SourceProduct;
}
...
router.post('/extract-url', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (info) {
      ...
      const saved = d.prepare('SELECT * FROM source_products WHERE id = ?').get(r.lastInsertRowid) as Record<string, unknown>;
...
router.post('/import-manual', (req: Request, res: Response) => {
  ...
  const tx = d.transaction(() => {
    for (const item of products) {
      ...
      imported.push(rowToProduct(d.prepare('SELECT * FROM my_products WHERE id = ?').get(pr.lastInsertRowid) as Record<string, unknown>));
    }
  });
  tx();
...
router.post('/import', (req: Request, res: Response) => {
  ...
  if (!sourceIds || !Array.isArray(sourceIds)) return res.status(400).json({ error: '请选择要导入的商品' });
  ...
  const tx = d.transaction(() => {
    for (const sourceId of sourceIds) {
      const source = getSource.get(parseInt(sourceId)) as Record<string, unknown> | undefined;
      if (!source) continue;
      const existing = checkExisting.get(parseInt(sourceId)) as { count: number };
      if (existing.count > 0) continue;

      const pricing = applyPricing((source.price as number) || 0);
