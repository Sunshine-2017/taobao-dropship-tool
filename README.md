...
1. **1688 选品**: 输入关键词，从 1688 搜索商品，展示价格、图片、店铺
2. **导入商品库**: 选中想要的商品，一键导入到本地库，自动计算售价（成本 × 倍率 + 固定加价）
3. **一键上架**: 在商品库勾选商品，填好淘宝类目，点「一键上架」
4. **自动登录淘宝**: Playwright 打开 Chromium，检测登录态，未登录则提示扫码，扫码后自动继续
5. **自动选类目**: 根据你填写的类目关键词，在淘宝 AI 类目页搜索并选中正确类目
6. **自动填表单**: 自动填写宝贝标题、价格、库存、品牌、包装方式、产地、运费模板
7. **自动上传图片**: 从 1688 商品页获取图片 URL → 下载到本地 → 通过淘宝图片上传 iframe 自动上传到发布页
8. **自动提交**: 点击提交按钮，检测发布成功或校验错误

## 你的角色

| 步骤 | 你做 | 程序做 |
|------|------|--------|
| 搜索商品 | 输入关键词搜索 | 从 1688 拉取商品列表 |
| 选择商品 | 勾选要上架的商品 | — |
| 填写类目 | 告诉程序淘宝类目（如 `茶>代用/花草/水果/再加工茶>组合型花茶`） | — |
| 登录淘宝 | 在弹出的 Chromium 窗口扫码登录 | 检测登录态，登录后自动继续 |
| 上架全流程 | — | 选类目、填标题、填价格、填库存、选品牌、上传图片、提交发布 |

你只需要到最后一步结束，在浏览器看发布结果。

## 程序不做的事

- ❌ **不绕开淘宝规则** — 用官方卖家中心接口，走正常发布流程
- ❌ **不存你的淘宝密码** — 扫码登录，cookie 存在本地
- ❌ **不碰你的钱** — 不处理交易、不代收代付
- ❌ **不批量发垃圾商品** — 正常速度操作，一次处理几个商品
