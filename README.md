...
| # | 步骤 | 谁来做 |
|---|------|--------|
| 1 | 在网页搜索1688商品 | **你** 输入关键词 → 程序拉商品列表 |
| 2 | 勾选要上架的商品 | **你** 勾选 |
| 3 | 填写淘宝类目 | **你** 告诉程序类目名（如 `茶>代用/花草/水果/再加工茶>组合型花茶`） |
| 4 | 一键上架 | 程序启动浏览器，自动导航到淘宝发布页 |
| 5 | 登录淘宝 | **你** 在浏览器窗口扫码登录（仅首次需要，之后自动复用） |
| 6 | 选择类目 | 程序根据你填的类目名，在淘宝类目系统搜索并选中正确的类目 |
| 7 | 填写发布表单 | 程序自动填宝贝标题、价格、库存、品牌、包装方式、产地、运费模板 |
| 8 | 上传图片 | 程序自动从1688下载商品图片 → 上传到淘宝发布页 |
| 9 | 提交发布 | 程序点击提交按钮，检测发布成功或报错信息 |

你只需要参与：搜商品 → 勾选 → 填类目 → 扫码。剩下全部自动完成。
